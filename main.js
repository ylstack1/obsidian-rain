...
// src/RaindropItemDetail.ts
var import_obsidian = require("obsidian");
var RaindropItemDetail = class {
  ...
  render(item) {
    ...
    (0, import_obsidian.setIcon)(addToObsidianBtn, "download");
    (0, import_obsidian.setTooltip)(addToObsidianBtn, "Add to Obsidian");
    addToObsidianBtn.onclick = async () => {
      ...
      try {
        await this.plugin.createRaindropNote({
          ...
          note: item.note || "",
          collectionId: ((_a6 = item.collection) == null ? void 0 : _a6.$id) || 0,
          collectionName: ((_b = item.collection) == null ? void 0 : _b.title) || "Unsorted"
        });
        new import_obsidian.Notice(`Created note for: ${item.title}`, 3e3);
      } catch (error) {
        new import_obsidian.Notice(`Failed to create note: ${error.message}`, 5e3);
        console.error("Error creating note from item detail:", error);
      }
    };
    const openLinkBtn = actions.createEl("button", { cls: "clickable-icon" });
    (0, import_obsidian.setIcon)(openLinkBtn, "external-link");
    (0, import_obsidian.setTooltip)(openLinkBtn, "Open Link in Browser");
    ...
    (0, import_obsidian.setTooltip)(editBtn, "Edit Bookmark (Not Implemented)");
    editBtn.onclick = () => new import_obsidian.Notice("Edit functionality coming soon!", 2e3);
    ...
    collectionEl.createEl("span", { text: ((_a5 = item.collection) == null ? void 0 : _a5.title) || "Unsorted", cls: "raindrop-detail-value" });
...
// src/RaindropView.ts
var RAINDROP_VIEW_TYPE = "raindrop-view";
var RaindropView = class extends import_obsidian2.ItemView {
  constructor(leaf, plugin) {
    super(leaf);
    this.collections = [];
    this.collectionMap = /* @__PURE__ */ new Map();
    this.collectionHierarchy = /* @__PURE__ */ new Map();
  ...
  async fetchAndRenderCollections(container, initialLoad = false) {
  ...
  buildHierarchy() {
    ...
    this.collections.forEach((col) => {
      var _a5, _b;
      const parentId = ((_a5 = col.parent) == null ? void 0 : _a5.$id) || 0;
      if (!this.collectionHierarchy.has(parentId)) {
        this.collectionHierarchy.set(parentId, []);
      }
      (_b = this.collectionHierarchy.get(parentId)) == null ? void 0 : _b.push(col._id);
    });
  }
  ...
  async setActiveCollection(collectionId) {
  ...
  renderItemList(items, container) {
    ...
    items.forEach((item) => {
      ...
      itemEl.setAttr("data-item-id", item._id);
      ...
      (0, import_obsidian2.setTooltip)(iconEl, item.type.charAt(0).toUpperCase() + item.type.slice(1));
  ...
  renderCollectionNode(collectionId, parentEl) {
    const collection = this.collectionMap.get(collectionId);
    if (!collection) return;
    const li = parentEl.createEl("li", { cls: "tree-item is-collapsed" });
    ...
    itemDiv.onclick = (e) => {
      ...
      (0, import_obsidian2.setIcon)(iconEl, li.hasClass("is-collapsed") ? "folder" : "folder-open");
    };
...
async function fetchMetadataFromUrl(url) {
  ...
  try {
    const html3 = await (0, import_obsidian4.request)({ url });
    const $2 = load(html3);
    let title = $2("title").text() || "";
    let description = $2('meta[name="description"]').attr("content") || $2('meta[property="og:description"]').attr("content") || "";
    let cover = $2('meta[property="og:image"]').attr("content") || $2('meta[name="twitter:image"]').attr("content") || "";
    if (!title) {
...
var AddNewBookmarkModal = class extends import_obsidian6.Modal {
  ...
  async onOpen() {
    ...
    let urlTextComponent;
    new import_obsidian6.Setting(contentEl).setName("URL").setDesc("Enter the URL of the bookmark to add.").setClass("setting-item-stacked").addText((text3) => {
      urlTextComponent = text3;
      text3.setPlaceholder("https://example.com").setValue(this.url).onChange((value) => {
        this.url = value.trim();
      ...
      button.setButtonText("Fetch Metadata").setCta().setDisabled(!this.url).onClick(async () => {
        if (!this.url) {
          new import_obsidian6.Notice("Please enter a URL first.", 3e3);
          return;
        }
        ...
        try {
          ...
          new import_obsidian6.Notice("Metadata fetched successfully!", 3e3);
        } catch (error) {
          new import_obsidian6.Notice("Failed to fetch metadata. Please check the URL.", 5e3);
    ...
    new import_obsidian6.Setting(contentEl).setName("Title").setClass("setting-item-stacked").addText((text3) => {
      this.titleTextComponent = text3;
      text3.setPlaceholder("Title").setValue(this.title).onChange((value) => {
        this.title = value.trim();
      });
      text3.inputEl.style.width = "100%";
    });
    new import_obsidian6.Setting(contentEl).setName("Excerpt/Description").setClass("setting-item-stacked").addTextArea((text3) => {
      this.descriptionTextComponent = text3;
      text3.setPlaceholder("Short description or excerpt").setValue(this.description).onChange((value) => {
        this.description = value.trim();
      });
      text3.inputEl.style.width = "100%";
    });
    new import_obsidian6.Setting(contentEl).setName("Cover Image URL").setClass("setting-item-stacked").addText((text3) => {
      text3.setPlaceholder("Optional image URL").setValue(this.cover).onChange((value) => {
        this.cover = value.trim();
      });
      text3.inputEl.style.width = "100%";
    });
    new import_obsidian6.Setting(contentEl).setName("Note").setDesc("Your personal note for this bookmark.").setClass("setting-item-stacked").addTextArea((text3) => {
      text3.setPlaceholder("Your personal note").setValue(this.note).onChange((value) => {
        this.note = value.trim();
      });
      text3.inputEl.style.width = "100%";
    });
    new import_obsidian6.Setting(contentEl).setName("Tags").setDesc("Comma-separated tags (e.g., tag1, tag2)").setClass("setting-item-stacked").addText((text3) => {
      text3.setPlaceholder("e.g., web, read-later").setValue(this.tags).onChange((value) => {
        this.tags = value.trim();
      });
      text3.inputEl.style.width = "100%";
    });
    new import_obsidian6.Setting(contentEl).setName("Collection").setDesc("Select the Raindrop collection to save this bookmark to.").addDropdown((dropdown) => {
      this.collectionSelect = dropdown;
      dropdown.addOption("0", "Unsorted");
      this.plugin.fetchAllUserCollections().then((collections) => {
        ...
        new import_obsidian6.Notice("Failed to load collections.", 7e3);
        console.error("Error loading collections in modal:", error);
      });
    });
    const buttonsEl = contentEl.createDiv({ cls: "modal-button-container" });
    new import_obsidian6.ButtonComponent(buttonsEl).setButtonText("Add Bookmark").setCta().onClick(async () => {
      if (!this.url || !this.title) {
        new import_obsidian6.Notice("URL and Title are required.", 3e3);
        return;
      }
      this.close();
    ...
    new import_obsidian6.ButtonComponent(buttonsEl).setButtonText("Cancel").onClick(() => {
      this.close();
    });
  }
  buildHierarchy() {
  ...
  getFullPath(collectionId) {
    var _a5;
    const segments = [];
    let currentId = collectionId;
    while (currentId !== void 0 && currentId !== 0) {
      const collection = this.collectionMap.get(currentId);
      if (!collection) break;
      segments.push(collection.title);
      currentId = (_a5 = collection.parent) == null ? void 0 : _a5.$id;
    }
    return segments.reverse().join(" > ");
  }
};
var RaindropToObsidian = class extends import_obsidian6.Plugin {
  ...
  onunload() {
    ...
    (_a5 = this.ribbonIconEl) == null ? void 0 : _a5.remove();
  ...
  getTemplateForType(type, options) {
    ...
    const shouldUseTypeTemplate = options.overrideTemplates || this.settings.contentTypeTemplateToggles[type] && ((_a5 = this.settings.contentTypeTemplates[type]) == null ? void 0 : _a5.trim()) !== "";
  ...
  generateFileName(raindrop, useRaindropTitleForFileName) {
    ...
    try {
      ...
      replacePlaceholder("collectionTitle", ((_a5 = raindrop.collection) == null ? void 0 : _a5.title) || "No Collection");
      ...
      new import_obsidian6.Notice("Error generating file name. Check console or template.");
  ...
  async fetchRaindrops(options) {
    ...
    const loadingNotice = new import_obsidian6.Notice("Starting Raindrop fetch...", 0);
    try {
      let allData = [];
      const perPage = 50;
      if (!this.settings.apiToken) {
        loadingNotice.hide();
        new import_obsidian6.Notice("Please configure your Raindrop.io API token in the plugin settings.", 1e4);
        return;
      }
      ...
      if (options.collections) {
        ...
        if (allCollections.length === 0) {
          ...
          loadingNotice.hide();
          new import_obsidian6.Notice("Error fetching user collections. Please check your API token and connection.", 1e4);
          return;
        }
        allCollections.forEach((col) => {
          ...
          collectionHierarchy.set(col._id, { title: col.title, parentId: (_a6 = col.parent) == null ? void 0 : _a6.$id });
        });
        ...
        if (unresolvedInputs.length > 0) {
          new import_obsidian6.Notice(`Could not find collections: ${unresolvedInputs.join(", ")}. Please check names or use IDs.`, 15e3);
        }
        if (resolvedCollectionIds.length === 0 && collectionInputs.length > 0) {
          loadingNotice.hide();
          new import_obsidian6.Notice("No valid collection IDs or names provided.", 5e3);
          return;
        }
      ...
      if (fetchMode === "collections") {
        for (const collectionId of resolvedCollectionIds) {
          ...
          while (hasMore) {
            ...
            if (!data2.result) {
              console.error(`API Error for collection ${collectionId}:`, data2);
              new import_obsidian6.Notice(`Error fetching collection: ${collectionNameForNotice}. Skipping.`, 7e3);
              hasMore = false;
              continue;
            }
            if (data2 == null ? void 0 : data2.items) {
              allData = allData.concat(data2.items);
              page++;
              hasMore = data2.items.length === perPage;
              console.log(`Fetched ${data2.items.length} items from collection ${collectionId}, page ${page}`);
              if (hasMore) {
                loadingNotice.setMessage(`Fetching from collection: ${collectionNameForNotice}, page ${page + 1}...`);
              }
            } else {
        ...
        if (options.tagMatchType === TagMatchTypes.ANY && options.apiFilterTags.length > 0) {
          ...
          for (const tag of tagsArray) {
            ...
            while (hasMore) {
              ...
              const data2 = response;
              if (!data2.result) {
                console.error(`API Error for tag ${tag}:`, data2);
                continue;
              }
              console.log(`API Response for tag ${tag}:`, {
                result: data2.result,
                itemCount: ((_a5 = data2 == null ? void 0 : data2.items) == null ? void 0 : _a5.length) || 0,
                totalCount: (data2 == null ? void 0 : data2.count) || 0
              });
              if (data2 == null ? void 0 : data2.items) {
                ...
                page++;
                hasMore = data2.items.length === perPage;
                console.log(`Fetched ${data2.items.length} items for tag ${tag}, page ${page}`);
                if (hasMore) {
                  loadingNotice.setMessage(`Fetching items with tag: ${tag}, page ${page + 1}...`);
                }
              } else {
          ...
          while (hasMore) {
            ...
            const data2 = response;
            if (!data2.result) {
              console.error("API Error for tag search:", data2);
              throw new Error(`API Error: ${JSON.stringify(data2)}`);
            }
            if (data2 == null ? void 0 : data2.items) {
              allData = allData.concat(data2.items);
              page++;
              hasMore = data2.items.length === perPage;
              console.log(`Fetched ${data2.items.length} items with tags, page ${page}`);
              if (hasMore) {
                loadingNotice.setMessage(`Fetching items with tags: ${searchParameterString}, page ${page + 1}...`);
              }
            } else {
        ...
        while (hasMore) {
          ...
          const data2 = response;
          if (!data2.result) {
            console.error("API Error for all items fetch:", data2);
            throw new Error(`API Error: ${JSON.stringify(data2)}`);
          }
          if (data2 == null ? void 0 : data2.items) {
            allData = allData.concat(data2.items);
            page++;
            hasMore = data2.items.length === perPage;
            console.log(`Fetched ${data2.items.length} items, page ${page}`);
            if (hasMore) {
              loadingNotice.setMessage(`Fetching all items, page ${page + 1}...`);
            }
          } else {
      ...
      if (allData.length === 0) {
        if (resolvedCollectionIds.length > 0 || searchParameterString || options.tagMatchType === TagMatchTypes.ANY && options.apiFilterTags.length > 0) {
          loadingNotice.hide();
          new import_obsidian6.Notice("No raindrops found matching your criteria.", 5e3);
        } else {
          loadingNotice.hide();
          new import_obsidian6.Notice("No raindrops found in your account.", 5e3);
        }
      } else {
        ...
        if (options.filterType && options.filterType !== "all") {
          ...
          if (filteredData.length === 0) {
            new import_obsidian6.Notice(`No raindrops found matching type '${options.filterType}'.`, 5e3);
            loadingNotice.hide();
            return;
          }
        } else {
      ...
      new import_obsidian6.Notice(`Error fetching raindrops: ${errorMessage}`, 1e4);
  ...
  async processRaindrops(raindrops, vaultPath, appendTagsToNotes, useRaindropTitleForFileName, loadingNotice, options, collectionsData, resolvedCollectionIds = [], collectionIdToNameMap = /* @__PURE__ */ new Map()) {
    ...
    const baseTargetFolderPath = (vaultPath == null ? void 0 : vaultPath.trim()) ? (0, import_obsidian6.normalizePath)(vaultPath.trim()) : (0, import_obsidian6.normalizePath)("");
    const collectionHierarchy = /* @__PURE__ */ new Map();
    ...
    try {
      ...
      for (const raindrop of raindrops) {
        const collectionId = ((_b = (_a5 = raindrop.collection) == null ? void 0 : _a5.$id) == null ? void 0 : _b.toString()) || "uncategorized";
      ...
      new import_obsidian6.Notice(summary, 7e3);
    } catch (error) {
      loadingNotice.hide();
      let errorMsg = "An unknown error occurred";
      if (error instanceof Error) errorMsg = error.message;
      else if (typeof error === "string") errorMsg = error;
      new import_obsidian6.Notice(`Error processing raindrops: ${errorMsg}`, 1e4);
  ...
  async processRaindrop(raindrop, baseTargetFolderPath, settingsFMTags, options, loadingNotice, processed, total, collectionHierarchy, collectionIdToNameMap) {
    ...
    try {
      ...
      if ((_a5 = raindrop.collection) == null ? void 0 : _a5.$id) {
        ...
        if (pathSegments.length > 0) {
          ...
          individualNoteTargetFolderPath = (0, import_obsidian6.normalizePath)(`${baseTargetFolderPath}/${collectionSubPath}`);
        }
      }
      ...
      const filePath = (0, import_obsidian6.normalizePath)(`${individualNoteTargetFolderPath}/${generatedFilename}.md`);
...
`;
  ...
  renderTemplate(template, data2) {
    const enhancedData = {
      ...data2,
      url: data2.link || "",
      domain: this.getDomain(data2.link || ""),
...
${indent}- ${this.formatYamlValue(item, indentLevel + 1)}`).join("");
    ...
    if (typeof value === "object") {
      ...
      return entries.map(([key, val2]) => {
        const formattedValue = this.formatYamlValue(val2, indentLevel + 1);
  ...
  getNestedProperty(obj, path) {
    return path.split(".").reduce((current, prop2) => {
      return current && current[prop2] !== void 0 ? current[prop2] : void 0;
    }, obj);
  }
  ...
  async fetchAllUserCollections() {
    ...
    if (!this.settings.apiToken) {
      ...
      new import_obsidian6.Notice("API token not set. Cannot fetch collections for modal.", 5e3);
      return [];
    }
    const baseApiUrl = "https://api.raindrop.io/rest/v1";
    const fetchOptions = {
    ...
    try {
      ...
      if (allCollections.length > 0 && uniqueCollections.length === 0 && (((_a5 = rootData == null ? void 0 : rootData.items) == null ? void 0 : _a5.length) || ((_b = nestedData == null ? void 0 : nestedData.items) == null ? void 0 : _b.length))) {
      ...
      new import_obsidian6.Notice("Failed to load your Raindrop.io collections for selection.", 7e3);
      return [];
    }
  }
  /**
   ...
   */
  async createRaindropNote(data2) {
    const loadingNotice = new import_obsidian6.Notice(`Creating note for: ${data2.title}...`, 0);
    try {
      const now = (/* @__PURE__ */ new Date()).toISOString();
      const simulatedRaindrop = {
        _id: Date.now(),
        // Use timestamp as unique ID for manually created bookmarks
        title: data2.title,
        excerpt: data2.excerpt,
        note: data2.note,
        link: data2.url,
        cover: data2.cover,
        created: now,
        ...
        tags: data2.tags,
        type: "link",
        // Always 'link' for manually added bookmarks
        collection: {
          $id: data2.collectionId,
          title: data2.collectionName
        },
        // Required fields for processRaindrop
        domain: new URL(data2.url).hostname,
        media: [],
        sort: 0,
        hidden: false,
      ...
      const collectionHierarchy = /* @__PURE__ */ new Map();
      const collectionIdToNameMap = /* @__PURE__ */ new Map();
      if (data2.collectionId && data2.collectionId !== 0) {
        try {
          ...
          allUserCollections.forEach((col) => {
            ...
            collectionHierarchy.set(col._id, {
              title: col.title,
              parentId: (_a5 = col.parent) == null ? void 0 : _a5.$id
            });
          });
        } catch (error) {
      ...
      if (data2.collectionId && !collectionIdToNameMap.has(data2.collectionId)) {
        collectionIdToNameMap.set(data2.collectionId, data2.collectionName);
      }
      const options = {
        vaultPath: data2.vaultPath,
        collections: "",
        apiFilterTags: "",
        includeSubcollections: false,
        appendTagsToNotes: "",
        // Tags are already in the raindrop object
      ...
      const settingsFMTags = data2.tags || [];
      const result = await this.processRaindrop(
        simulatedRaindrop,
        data2.vaultPath || this.settings.defaultFolder,
        settingsFMTags,
        options,
        loadingNotice,
        1,
      ...
      loadingNotice.hide();
      if (result.success) {
        new import_obsidian6.Notice(`\u2713 Successfully created note: ${data2.title}`, 5e3);
      } else {
        new import_obsidian6.Notice(`\u26A0 Note creation completed with warnings for: ${data2.title}`, 5e3);
      }
    } catch (error) {
      loadingNotice.hide();
      const errorMessage = error instanceof Error ? error.message : "Unknown error";
      new import_obsidian6.Notice(`\u2717 Failed to create note: ${errorMessage}`, 7e3);
  ...
  async fetchCollectionItems(collectionId) {
    ...
    while (hasMore) {
      ...
      try {
        ...
        if (!data2.result) {
          console.error(`API Error for collection ${collectionId}:`, data2);
          new import_obsidian6.Notice(`Error fetching items for collection ID: ${collectionId}.`, 7e3);
          hasMore = false;
          continue;
        }
        if (data2 == null ? void 0 : data2.items) {
        ...
        new import_obsidian6.Notice(`Network error fetching items for collection ID: ${collectionId}.`, 7e3);
        hasMore = false;
      }
    }
    return allItems;
  ...
  async fetchSingleRaindrop(itemId, vaultPath, appendTags) {
    ...
    if (!itemId) {
      new import_obsidian6.Notice("Invalid Item ID provided for Quick Import.", 5e3);
      return;
    }
    const loadingNotice = new import_obsidian6.Notice(`Fetching Raindrop item ID: ${itemId}...`, 0);
    const baseApiUrl = "https://api.raindrop.io/rest/v1";
    const fetchOptions = {
      method: "GET",
    ...
    try {
      ...
      const data2 = response;
      let raindropItem;
      if (data2.result && data2.item) {
        raindropItem = data2.item;
      } else if (data2.result && data2.items && data2.items.length > 0) {
        raindropItem = data2.items[0];
        console.warn(`Single raindrop fetch for ID ${itemId} returned an items array. Using the first item.`);
      }
      if (!raindropItem) {
        loadingNotice.hide();
        const errorMsg = data2.result === false ? data2.errorMessage || "API indicated failure." : "Item not found or invalid response.";
        new import_obsidian6.Notice(`Failed to fetch Raindrop item ${itemId}: ${errorMsg}`, 7e3);
        console.error(`Failed to fetch Raindrop item ${itemId}:`, data2);
        return;
      }
      ...
      if ((_a5 = raindropItem.collection) == null ? void 0 : _a5.$id) {
      ...
      if (error instanceof Error) errorMessage = error.message;
      new import_obsidian6.Notice(`Error during Quick Import of item ${itemId}: ${errorMessage}`, 1e4);
      console.error(`Error quick importing Raindrop ID ${itemId}:`, error);
    }
  }
};
var RaindropToObsidianSettingTab = class extends import_obsidian6.PluginSettingTab {
  constructor(app, plugin) {
    super(app, plugin);
    this.plugin = plugin;
  }
  display() {
    ...
    const apiTokenSetting = new import_obsidian6.Setting(containerEl).setName("Raindrop.io API Token").setDesc('Create a "Test Token" from your Raindrop.io Apps settings.').addText((text3) => {
      text3.setPlaceholder("Enter your API token").setValue(this.plugin.settings.apiToken).onChange(async (value) => {
    ...
    new import_obsidian6.Setting(containerEl).setName("Default Vault Save Location").setDesc("Specify the default folder for imported notes (e.g., Imports/Raindrops). Leave blank for vault root.").addText((text3) => {
      text3.setPlaceholder("e.g., Raindrops/").setValue(this.plugin.settings.defaultFolder).onChange(async (value) => {
        this.plugin.settings.defaultFolder = value;
        await this.plugin.saveSettings();
      });
      text3.inputEl.style.width = "100%";
    });
    const fileNameTemplateSetting = new import_obsidian6.Setting(containerEl).setName("Filename Template").setDesc('Define the filename for notes when "Use Raindrop Title" is enabled. Placeholders: {{title}}, {{id}}, {{collectionTitle}}, {{date}} (YYYY-MM-DD).').addText((text3) => {
      text3.setPlaceholder("{{title}}").setValue(this.plugin.settings.fileNameTemplate).onChange(async (value) => {
    ...
    new import_obsidian6.Setting(containerEl).setName("Banner Frontmatter Field Name").setDesc("Customize the frontmatter field name for the banner/cover image (default: banner).").addText((text3) => {
      text3.setPlaceholder("banner").setValue(this.plugin.settings.bannerFieldName).onChange(async (value) => {
        this.plugin.settings.bannerFieldName = value;
        await this.plugin.saveSettings();
      });
    });
    containerEl.createEl("h2", { text: "\u{1F3A8} User Interface" });
    new import_obsidian6.Setting(containerEl).setName("Show Ribbon Icon").setDesc("Toggle the Make It Rain ribbon icon in the Obsidian sidebar.").addToggle((toggle) => {
    ...
    new import_obsidian6.Setting(containerEl).setName("Enable Template System").setDesc("Use custom templates for formatting imported notes. If disabled, a basic note structure will be used.").addToggle((toggle) => {
    ...
    if (this.plugin.settings.isTemplateSystemEnabled) {
      ...
      new import_obsidian6.Setting(containerEl).setDesc("This template is used if no content-type specific template is active or defined below.").setClass("setting-item-stacked").addTextArea((text3) => {
        text3.setPlaceholder("Enter your default Handlebars template here...").setValue(this.plugin.settings.defaultTemplate).onChange(async (value) => {
          this.plugin.settings.defaultTemplate = value;
          await this.plugin.saveSettings();
        });
        text3.inputEl.rows = 15;
        text3.inputEl.style.width = "100%";
        text3.inputEl.style.fontFamily = "monospace";
      }).addButton((button) => {
        button.setButtonText("Reset to Default").setIcon("undo").setTooltip("Reset this template to its original default value").onClick(async () => {
          ...
          await this.plugin.saveSettings();
          this.display();
          new import_obsidian6.Notice("Default template has been reset.");
        });
      });
      ...
      for (const type of contentTypes) {
        ...
        new import_obsidian6.Setting(containerEl).setName(`Enable ${type} Template`).setDesc(`Use a custom template for "${type}" items.`).addToggle((toggle) => {
        ...
        if (this.plugin.settings.contentTypeTemplateToggles[typeKey]) {
          new import_obsidian6.Setting(containerEl).setDesc(`Template for "${type}" content. Leave empty to use the default template.`).setClass("setting-item-stacked").addTextArea((text3) => {
            text3.setPlaceholder(`Enter template for ${type} items...`).setValue(this.plugin.settings.contentTypeTemplates[typeKey]).onChange(async (value) => {
              this.plugin.settings.contentTypeTemplates[typeKey] = value;
              await this.plugin.saveSettings();
            });
            text3.inputEl.rows = 10;
            text3.inputEl.style.width = "100%";
            text3.inputEl.style.fontFamily = "monospace";
          }).addButton((button) => {
            button.setButtonText("Reset").setIcon("undo").setTooltip(`Reset ${type} template to its original default`).onClick(async () => {
              if (DEFAULT_SETTINGS.contentTypeTemplates[typeKey]) {
                ...
                await this.plugin.saveSettings();
                this.display();
                new import_obsidian6.Notice(`${type.charAt(0).toUpperCase() + type.slice(1)} template has been reset.`);
              } else {
                new import_obsidian6.Notice(`Error: Default template for ${type} not found.`, 7e3);
              }
            });
          });
        }
  ...
  async verifyApiToken() {
    ...
    if (!apiToken) {
      new import_obsidian6.Notice("Please enter an API token first.", 5e3);
      return;
    }
    new import_obsidian6.Notice("Verifying API token...", 3e3);
    const baseApiUrl = "https://api.raindrop.io/rest/v1";
    const fetchOptions = {
      headers: {
    ...
    try {
      ...
      if (data2.result) {
        new import_obsidian6.Notice("API Token is valid!", 5e3);
      } else {
        const errorMessage = data2.message || data2.error || "Invalid API token or connection issue.";
        new import_obsidian6.Notice(`API Token verification failed: ${errorMessage}`, 1e4);
        console.error("API Token verification failed:", data2);
      }
    } catch (error) {
      let errorMsg = "An error occurred during token verification.";
      if (error instanceof Error) errorMsg = error.message;
      else if (typeof error === "string") errorMsg = error;
      new import_obsidian6.Notice(`API Token verification failed: ${errorMsg}`, 1e4);
